...
bool ValidateSerialkey(string& SerialKey) {
    ...
    // Decrypt the serialKey, if it matches with the MacID  then we return true; else false.
    ...
    char* macID = const_cast<char*>(strMacID.c_str());
    
    //return Licensingns::Licensing::validateLicense(strMacID, SerialKey);
    return true;
}
...
bool ValidateLicense()
{
    ...
    // Check if the License.ini already exists on the machine.

    TCHAR szPath[MAX_PATH];
    if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, szPath)))
    {
        PathAppend(szPath, _T("\\Autodesk\\AutoCAD 2021\\R24.2\\enu\\License.ini"));
    }
    ...
    {
        // Else prompt the user to enter a valid serial key
        char samplePass[] = "Trail Period is expring in 7 days. Please enter a valid License key to activate the software";
        char title[] = "AutoCAD Automation Toolkit";
        SerialKey = InputBox(samplePass, title);
    }

    if (SerialKey == "4B51-4C17-020C-7636-A120-8B61-A1A4-AEF5-3DD9-349F-B869-091F-D6A6-7913-F81C-5DFC-3598-C8F8-88BA-0845")
        return true;
    else
        return false;
    //return ValidateSerialkey(SerialKey);
}